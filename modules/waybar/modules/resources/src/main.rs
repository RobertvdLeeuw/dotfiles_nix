...
fn format_info(device_type: &Device, usage: i32, temp: i32, mem: i32) -> String {
    ...
    // mono.
    let body = format!("{icon}  <b>{usage}%</b> {spacer}{temp}°C <b>|</b> {mem}%   ",
                       icon=icon, usage=usage, temp=temp, mem=mem,
...
fn jq_select(mut process: Child, jq_path: &str) -> String {
    let stdout = process.stdout.take().expect("Failed to get stdout from prev.");
    ...
    let result = String::from_utf8(jq_child
        .wait_with_output()
        .expect("Failed to get jq output")
        .stdout).expect("Failed to cast stdout to string.");
...
fn get_device_temp(jq_path: &str) -> i32 {
    ...
    jq_select(temp, jq_path).parse::<f32>().expect("Failed parsing device temp to float.") as i32
}
...
fn get_gpu_info() -> (i32, i32, i32) {
    ...
    let info = jq_select(rocm, ".\"card0\"| .\"GPU use (%)\", .\"GPU Memory Allocated (VRAM%)\"")
        .split("\n")
        .map(|x| x
            .trim()
            .replace("\"","")
...
fn main() {
    ...
    assert_eq!(args.len(), 2,
        "Wrong number of args (expected 2, got {}), 'resources <device>'", args.len()
    );
